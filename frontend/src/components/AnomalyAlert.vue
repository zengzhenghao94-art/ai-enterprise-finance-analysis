  ...
  <div class="bg-white rounded-xl border border-gray-200 p-4">
    <div class="flex items-center justify-between mb-3">
      ...
      <button
        @click="runDetection"
        :disabled="detecting"
        class="px-3 py-1.5 bg-orange-100 text-orange-700 rounded-lg text-xs font-medium
               hover:bg-orange-200 disabled:opacity-50 disabled:cursor-not-allowed
    ...
    >
      ...
      <span v-if="backendSummary.high" class="text-red-600 font-medium">🔴 {{ backendSummary.high }} 高</span>
    ...
    >
      ...
      <div>未检测到异常</div>
      <div class="text-xs text-gray-300 mt-1">点击"重新检测"运行 ML 异常检测</div>
    </div>
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-3 text-xs mt-1">
              <span class="text-gray-600">
                实际值: <strong class="text-gray-800">{{ formatValue(a.actual_value, a.metric_name) }}</strong>
              </span>
            ...
            <!-- LLM 解释 / 数据来源说明 -->
            <div v-if="a.description" class="text-xs text-gray-500 mt-1.5 leading-relaxed">
              💬 {{ a.description }}
            </div>
